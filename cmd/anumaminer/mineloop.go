...
import (
	...
	"github.com/AnumaNetwork/anumad-testnet/cmd/anumaminer/templatemanager"
	...
	"github.com/AnumaNetwork/anumad-testnet/domain/consensus/utils/pow"
...
func getBlockForMining(mineWhenNotSynced bool) (*externalapi.DomainBlock, *pow.State) {
	...
	for {
		...
		if !isSynced && !mineWhenNotSynced {
			if shouldLog {
				log.Warnf("Anumad is not synced. Skipping current block template")
			}
...
func templatesLoop(client *minerClient, miningAddr util.Address, errChan chan error) {
	getBlockTemplate := func() {
		template, err := client.GetBlockTemplate(miningAddr.String(), "anumaminer-"+version.Version())
		if nativeerrors.Is(err, router.ErrTimeout) {
