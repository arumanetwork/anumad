...
// 2. A wrapper that implements isAnumadMessage_Payload, having a single field pointing to the concrete command
//    bearing the name of the command with `AnumadMessage_` prefix and `Request` suffix
...
// generateAnumadMessage generates a wrapped AnumadMessage with the given `commandValue`
func generateAnumadMessage(commandValue reflect.Value, commandDesc *commandDescription) (*protowire.AnumadMessage, error) {
	commandWrapper := reflect.New(commandDesc.typeof)
	unwrapCommandValue(commandWrapper).Set(commandValue)

	anumadMessage := reflect.New(reflect.TypeOf(protowire.AnumadMessage{}))
	anumadMessage.Elem().FieldByName("Payload").Set(commandWrapper)
	return anumadMessage.Interface().(*protowire.AnumadMessage), nil
}
