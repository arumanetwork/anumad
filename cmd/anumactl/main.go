...
import (
	...
	"github.com/AnumaNetwork/anumad-testnet/infrastructure/network/rpcclient/grpcclient"
)
...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		anumadMessage, err := client.Post(&protowire.AnumadMessage{Payload: &protowire.AnumadMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := anumadMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	anumadMessage := &protowire.AnumadMessage{}
	err := protojson.Unmarshal([]byte(response), anumadMessage)
	if err != nil {
	...
	return marshalOptions.Format(anumadMessage)
}
