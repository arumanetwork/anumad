...
func SecToSompi(amount string) (uint64, error) {
	...
	if constants.SompiPerAnuma%10 != 0 {
		return 0, errors.Errorf("Unable to convert to sompi when SompiPerAnuma is not a multiple of 10")
	}

	decimalPlaces := int(math.Log10(constants.SompiPerAnuma))
	decimalStr := ""
...
func validateANUMAmountFormat(amount string) error {
