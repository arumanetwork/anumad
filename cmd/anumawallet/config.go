...
type createConfig struct {
	KeysFile          string `long:"keys-file" short:"f" description:"Keys file location (default: ~/.anumawallet/keys.json (*nix), %USERPROFILE%\\AppData\\Local\\Anumawallet\\key.json (Windows))"`
...
func parseCommandLine() (subCommand string, config interface{}) {
	...
	parser.AddCommand(balanceSubCmd, "Shows the balance of a public address",
		"Shows the balance for a public address in Anuma", balanceConf)

	sendConf := &sendConfig{DaemonAddress: defaultListen}
	parser.AddCommand(sendSubCmd, "Sends a Anuma transaction to a public address",
		"Sends a Anuma transaction to a public address", sendConf)
	...
	parser.AddCommand(createUnsignedTransactionSubCmd, "Create an unsigned Anuma transaction",
		"Create an unsigned Anuma transaction", createUnsignedTransactionConf)
