...
Clients are expected to build RequestMessages and wrap them in AnumadMessage. (see messages.proto)

Having received a RequestMessage, (wrapped in a AnumadMessage) the RPC server will respond with a
ResponseMessage (likewise wrapped in a AnumadMessage) respective to the original RequestMessage.
...
GetCurrentNetworkRequestMessage requests the network anumad is currently running against.
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| payAddress | [string](#string) |  | Which anuma address should the coinbase block reward transaction pay into |
...
| ----- | ---- | ----- | ----------- |
| block | [RpcBlock](#protowire.RpcBlock) |  |  |
| isSynced | [bool](#bool) |  | Whether anumad thinks that it&#39;s synced. Callers are discouraged (but not forbidden) from solving blocks when anumad is not synced. That is because when anumad isn&#39;t in sync with the rest of the network there&#39;s a high chance the block will never be accepted, thus the solving effort would have been wasted. |
| error | [RPCError](#protowire.RPCError) |  |  |
...
GetPeerAddressesRequestMessage requests the list of known anumad addresses in the
...
currently connected to this anumad.
...
| isOutbound | [bool](#bool) |  | Whether this anumad initiated the connection |
| timeOffset | [int64](#int64) |  |  |
| userAgent | [string](#string) |  |  |
| advertisedProtocolVersion | [uint32](#uint32) |  | The protocol version that this peer claims to support |
| timeConnected | [int64](#int64) |  | The timestamp of when this peer connected to this anumad |
...
AddPeerRequestMessage adds a peer to anumad&#39;s outgoing connection list.
This will, in most cases, result in anumad connecting to said peer.
...
parent chain from some startHash to this anumad&#39;s current virtual
...
GetBlockCountRequestMessage requests the current number of blocks in this anumad.
...
ShutDownRequestMessage shuts down this anumad.
...
This call is only available when this anumad was started with `--utxoindex`
...
GetUtxosByAddressesRequestMessage requests all current UTXOs for the given anumad addresses
