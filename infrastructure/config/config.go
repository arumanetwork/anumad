...
const (
	...
	sampleConfigFilename    = "sample-anumad.conf"
	defaultMaxUTXOCacheSize = 5_000_000_000
	defaultProtocolVersion  = 5
)

var (
	// DefaultAppDir is the default home directory for anumad.
	DefaultAppDir = util.AppDir("anumad", false)
...
// Flags defines the configuration options for anumad.
//
...
type Flags struct {
	...
	MinRelayTxFee                   float64       `long:"minrelaytxfee" description:"The minimum transaction fee in ANUM/kB to be considered a non-zero fee."`
...
// Config defines the configuration options for anumad.
//
// See loadConfig for details on the configuration load process.
type Config struct {
	*Flags
...
// DefaultConfig returns the default anumad configuration
func DefaultConfig() *Config {
	config := &Config{Flags: defaultFlags()}
	config.NetworkFlags.ActiveNetParams = &dagconfig.TestnetParams
	return config
}
...
//
// The above results in anumad functioning properly without any config settings
...
// createDefaultConfig copies the file sample-anumad.conf to the given destination path,
