...
func TestCreateDefaultConfigFile(t *testing.T) {
	...
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-anumad.conf")

	// Setup a temporary directory
	tmpDir, err := ioutil.TempDir("", "anumad")
	if err != nil {
		t.Fatalf("Failed creating a temporary directory: %v", err)
	}
	testpath := filepath.Join(tmpDir, "test.conf")

	// copy config file to location of anumad binary
	...
	tmpConfigFile := filepath.Join(appPath, "sample-anumad.conf")
