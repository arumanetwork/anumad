...
import (
	...
	"github.com/arumanetwork/anumad/domain/consensus/utils/subnetworks"
...
// Outpoint defines a anuma data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new anuma transaction outpoint point with the
...
// TxIn defines a anuma transaction input.
type TxIn struct {
...
// NewTxIn returns a new anuma transaction input with the provided
...
// TxOut defines a anuma transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new anuma transaction output with the provided
...
// MsgTx implements the Message interface and represents a anuma tx message.
