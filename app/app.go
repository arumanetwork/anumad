...
import (
	...
	"github.com/arumanetwork/anumad/infrastructure/db/database"
	"github.com/arumanetwork/anumad/infrastructure/db/database/ldb"
	"github.com/arumanetwork/anumad/infrastructure/logger"
	"github.com/arumanetwork/anumad/infrastructure/os/execenv"
	"github.com/arumanetwork/anumad/infrastructure/os/limits"
	"github.com/arumanetwork/anumad/infrastructure/os/signal"
	"github.com/arumanetwork/anumad/infrastructure/os/winservice"
	"github.com/arumanetwork/anumad/util/panics"
	"github.com/arumanetwork/anumad/util/profiling"
...
var serviceDescription = &winservice.ServiceDescription{
	Name:        "anumadsvc",
	DisplayName: "Anumad Service",
	Description: "Downloads and stays synchronized with the Anuma blockDAG and " +
		"provides DAG services to applications.",
}

type anumadApp struct {
	cfg *config.Config
}

// StartApp starts the anumad app, and blocks until it finishes running
func StartApp() error {
...
func (app *anumadApp) main(startedChan chan<- struct{}) error {
	...
	if err != nil {
		log.Errorf("Unable to start anumad: %+v", err)
		return err
	}

	defer func() {
		log.Infof("Gracefully shutting down anumad...")
		...
		log.Infof("Anumad shutdown complete")
	}()
