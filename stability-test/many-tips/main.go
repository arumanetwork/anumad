...
import (
	...
	"github.com/arumanetwork/anumad/domain/consensus/utils/mining"
	"github.com/arumanetwork/anumad/util"
	"github.com/kaspanet/go-secp256k1"

	"github.com/arumanetwork/anumad/stability-tests/common"
	"github.com/arumanetwork/anumad/stability-tests/common/rpc"
...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("anumad-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("anumad datadir: %s", dataDir)

	anumadCmd, err := common.StartCmd("ANUMAD",
		"anumad",
	...
	spawn("startNode-anumadCmd.Wait", func() {
		err := anumadCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("anumadCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes anumad closes ungracefully
				log.Errorf("anumadCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
	...
	return func() {
		...
		killWithSigterm(anumadCmd, "anumadCmd")
...
func mineLoopUntilHavingOnlyOneTipInDAG(rpcClient *rpc.Client, miningAddress util.Address) error {
	...
	anumaMinerCmd, err := common.StartCmd("MINER",
		"anumaminer",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
		"-s", rpcAddress,
	...
	spawn("anuma-miner-Cmd.Wait", func() {
		err := anumaMinerCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
	...
	atomic.StoreUint64(&shutdown, 1)
	killWithSigterm(anumaMinerCmd, "anumaMinerCmd")
	return nil
}
