...
func startNodes() (teardown func(), err error) {
	...
	syncerDataDir, err := common.TempDir("anumad-datadir-syncer")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCER datadir: %s", syncerDataDir)

	syncedDataDir, err := common.TempDir("anumad-datadir-synced")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCED datadir: %s", syncedDataDir)

	syncerCmd, err := common.StartCmd("ANUMAD-SYNCER",
		"anumad",
	...
	syncedCmd, err := common.StartCmd("ANUMAD-SYNCED",
		"anumad",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
